 ...
 * print only the message: a raw `err.stack` leaks internal file paths into the
 * operator's terminal/logs (audit #156). Set `SHORTWIND_DEBUG=1` to surface the
 * full stack when diagnosing.
 */
...
export async function run(argv: string[] = process.argv): Promise<void> {
  ...
  // command runs (#21). It sits at the top of the *root* entry because this is
  // the only path every user takes: the hosting namespace that installed the
  // skill is gone from this CLI, so a shim living inside it would have been
